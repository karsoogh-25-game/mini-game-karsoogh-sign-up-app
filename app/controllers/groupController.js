...
exports.getMyGroup = async (req, res) => {
    ...
    try {
        ...
        const group = await Group.findByPk(membership.groupId, {
            attributes: ['id', 'name', 'code', 'walletCode', 'score', 'color', 'leaderId'],
            include: [
        ...
        const filteredGroups = allGroups.filter(g => {
            return g.members && g.members.length > 0 && g.members.every(member => member.gender === userGender);
        });

        let rank = null;
        const userGroupInFilteredList = filteredGroups.find(g => g.id === group.id);

        if (userGroupInFilteredList) {
            const distinctScores = [...new Set(filteredGroups.map(g => g.score))].sort((a, b) => b - a);
            rank = distinctScores.indexOf(group.score) + 1;
        }
    ...
    try {
        ...
        if (userGender) {
            filteredGroups = allGroups.filter(group => {
                return group.members && group.members.length > 0 && group.members.every(member => member.gender === userGender);
            });
        }
